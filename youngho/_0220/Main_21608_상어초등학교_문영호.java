...
// 구현문제 답지보고 했다.
// 자료구조를 생각하는데 힘들었음
// 1단계 -> 2단계 -> 3단계 를 따로 처리하려했지만
// 어차피 순서는 정해져있고, 한개가 들어간 뒤 다음 사람이 들어가니
// 1번조건, 2번조건, 3번조건을 다함께
// 체크할 때 상하좌우로 좋아하는사람 있는지 체크
// 옆에 빈곳 체크
// 둘다 같다면 위쪽 여기도 같다면 위쪽 에서 오른쪽
// if( (maxLike < like) ||
// (maxLike == like && ( maxEmpty < empty ) || 동률이면 -> 오른쪽
// ( maxLike == like && maxEmpty == empty && ( row < bestRow  || (row == bestRow && col < bestCol))) )
// 이런식으로 짜주는데 if문 안에  이렇게 많이쓴거 처음임


public class Main_21608_상어초등학교_문영호 {
    static int[][] students;
