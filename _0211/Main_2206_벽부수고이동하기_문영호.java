...
import java.util.*;
// 가중치가 똑같고 그래프에서 최단 거리 - BFS
// 그중에서 벽을 한번 부실 수 있음
// 벽을 부신 얘랑, 안부신 얘랑 visited 체크하는게 달라야함

public class Main_2206_벽부수고이동하기_문영호 {
    static int N, M;
    static int[][] map;
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i < dx.length; i++){
                ...
                if(isIn(nx, ny) && map[nx][ny] == 1){ // nx, ny 가 벽일경우
                    if(xy[3] == 0 && !visited[1][nx][ny]){ // 벽이고 1 즉 벽 부신얘가 안갔으면 ㄱ
